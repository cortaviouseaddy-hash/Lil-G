...
- Private `/list` raid helper for owner-managed back-to-back raid queues: it drafts the queue announcement, tracks people who reply yes, and generates the next-group alert.
...
- To start a private raid list: `/list Mega Rayquaza`, then answer the start time. Add yes replies with `/list yes Corey`, and move to the next set with `/list next` or `/list done`.
